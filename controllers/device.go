...
func (c *DeviceController) Post() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		...
		if models.CheckDeviceName(v.DeviceName){
			c.Ctx.ResponseWriter.WriteHeader(403)
			c.Data["json"] = Response{403, 403,"设备名已经注册了", ""}
			c.ServeJSON()
			return
		}
		...
		c.Data["json"] = &Response{1, 1, "设备名注册失败", err.Error()}
	}
	c.ServeJSON()
}
...
func (c *DeviceController) GetAll() {
	...
	l, err := models.GetAllDevices(query, fields, sortby, order, offset, limit)
	if err != nil {
		c.Data["json"] = err.Error()
	} else {
		c.Data["json"] = l
...
func (c *DeviceController) GetDevicesByUserId() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if errorMessage := utils.CheckUserDevicePost(v.UserId, v.Limit, v.Offset); errorMessage != "ok"{
			c.Ctx.ResponseWriter.WriteHeader(403)
			c.Data["json"] = Response{403, 403,errorMessage, ""}
			c.ServeJSON()
			return
		...
		c.Data["json"] = &Response{1, 1, "设备结构解析失败", err.Error()}
	}
	c.ServeJSON()
}